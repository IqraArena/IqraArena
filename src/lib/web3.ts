...
export class Web3Service {
  ...
  async checkBalance(): Promise<{ balance: string; hasEnoughForGas: boolean }> {
    ...
    try {
      ...
      const minimumRequired = 0.00001; // Lowered for Base/ETH
  ...
  async registerUser(username: string): Promise<void> {
    ...
    if (!balanceCheck.hasEnoughForGas) {
      throw new Error(`لا يوجد رسوم غاز من عملة ETH كافية لديك بمحفظتك. لديك ${balanceCheck.balance} ETH وتحتاج على الأقل 0.00001 ETH. برجاء شحن محفظتك`);
    }
  ...
  async recordPagesRead(): Promise<void> {
    ...
    try {
      ...
      const transaction = prepareContractCall({
        ...
        params: [1n], // Changed from 5n to 1n - record 1 page at a time
      });
