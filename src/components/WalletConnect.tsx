...
import { Mail, AlertCircle, CheckCircle, Loader2, Copy, ArrowLeft, Moon, Sun, Fuel } from 'lucide-react';
...
export default function WalletConnect({ onConnected, onBack, darkMode, onToggleDarkMode }: WalletConnectProps) {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    try {
      // Step 1: Auto-fund the wallet with CELO for gas fees
      setFundingStatus('funding');
      setFundingMessage('جارٍ شحن محفظتك بعملة ETH...');

      const balanceCheck = await web3Service.checkBalance();

      if (!balanceCheck.hasEnoughForGas) {
        const fundResult = await fundingService.fundNewUser(walletAddress);

        if (!fundResult.success && !fundResult.alreadyFunded) {
          setFundingStatus('error');
          setFundingMessage(fundResult.message);
          setError('فشل شحن المحفظة. يرجى المحاولة مرة أخرى أو شحن محفظتك يدوياً.');
          setIsRegistering(false);
          return;
        }

        if (fundResult.alreadyFunded) {
          console.log('Wallet was already funded');
        } else {
          console.log('Wallet funded successfully, TX:', fundResult.txHash);
        }

        // Wait a moment for the funding tx to be indexed
        await new Promise(resolve => setTimeout(resolve, 2000));
      }

      setFundingStatus('funded');
      setFundingMessage('تم شحن المحفظة بنجاح! جارٍ التسجيل...');

      // Step 2: Register user on-chain
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            ...
            <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 text-sm">
              <div className="flex items-start gap-2 text-blue-800">
                <Fuel className="w-5 h-5 flex-shrink-0 mt-0.5" />
                <div className="flex-1">
                  <p className="text-blue-700 font-semibold">شحن تلقائي مجاني ⛽</p>
                  <p className="text-blue-600 mt-1">
                    سيتم شحن محفظتك تلقائياً بعملة ETH لتغطية رسوم المعاملات عند التسجيل.
                  </p>
                </div>
              </div>
            </div>
            ...
            {fundingStatus !== 'idle' && (
              <div className={`rounded-xl p-4 text-sm ${fundingStatus === 'funding' ? 'bg-amber-50 border border-amber-200' :
                fundingStatus === 'funded' ? 'bg-green-50 border border-green-200' :
                  'bg-red-50 border border-red-200'
                }`}>
                <div className={`flex items-center gap-2 ${fundingStatus === 'funding' ? 'text-amber-800' :
                  fundingStatus === 'funded' ? 'text-green-800' :
                    'text-red-800'
                  }`}>
                  {fundingStatus === 'funding' && <Loader2 className="w-5 h-5 animate-spin" />}
                  {fundingStatus === 'funded' && <CheckCircle className="w-5 h-5" />}
                  {fundingStatus === 'error' && <AlertCircle className="w-5 h-5" />}
                  <span className="font-medium">{fundingMessage}</span>
                </div>
              </div>
            )}
